...
@forelse($items as $item)<tr><td>{{ $item->vi_agent_name }}</td><td>{{ $item->mappedUser->name ?? '-' }}</td><td>{{ $item->is_active ? 'Active' : 'Inactive' }}</td><td>{{ $item->remarks ?: '-' }}</td><td><a href="{{ route('admin.ivr.agents.show',$item) }}" class="ti-btn ti-btn-sm ti-btn-info">View</a> <a href="{{ route('admin.ivr.agents.edit',$item) }}" class="ti-btn ti-btn-sm ti-btn-primary">Edit</a></td></tr>@empty<tr><td colspan="5" class="text-center">No agent mappings configured.</td></tr>@endforelse
</tbody></table></div></div>
@endsection
