...
<div class="block justify-between page-header md:flex">
    <div>
        <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.125rem] font-semibold"> Edit Service Address</h3>
    </div>
...
<div class="grid grid-cols-12 gap-6">
    <div class="xl:col-span-12 col-span-12">
        <div class="box">
            <form class="ti-custom-validation" action="{{ route('admin.service-addresses.update', $serviceAddress->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="box-body">
                    <div class="grid grid-cols-12 sm:gap-6">
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <select class="ti-form-select rounded-sm form-control-sm" name="product_id" id="product_id">
                                ...
                                <option value="{{ $product->id }}" {{ old('product_id', $serviceAddress->product_id) == $product->id ? 'selected' : '' }}>
                                    {{ $product->product }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <select class="ti-form-select rounded-sm form-control-sm" name="service_id" id="service_id">
                                ...
                                <option value="{{ $service->id }}" {{ old('service_id', $serviceAddress->service_id) == $service->id ? 'selected' : '' }}>
                                    {{ $service->service }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <input type="text" name="contact_person_name" class="ti-form-input rounded-sm form-control-sm" id="contact_person_name" value="{{ old('contact_person_name', $serviceAddress->contact_person_name) }}" required>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <input type="hidden" name="contact_country_code" id="contact_country_code"
                                value="{{ old('contact_country_code', $serviceAddress->contact_country_code) }}">
                            <input type="tel" name="contact_number" id="contact_number"
                                class="form-control w-full intl-phone-input"
                                value="{{ old('contact_number', $serviceAddress->contact_number) }}" required>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <select name="country_id" id="countrySelect"
                                ...
                                <option value="{{ $country->id }}"
                                    {{ old('country_id', $serviceAddress->city->state->country_id) == $country->id ? 'selected' : '' }}
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <select name="state_id" id="stateSelect"
                                ...
                                <option value="{{ $state->id }}"
                                    {{ old('state_id', $serviceAddress->city->state_id) == $state->id ? 'selected' : '' }}>
                                    {{ $state->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <select name="city_id" id="citySelect"
                                ...
                                <option value="{{ $city->id }}"
                                    {{ old('city_id', $serviceAddress->city_id) == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <input type="url" name="map_link" class="ti-form-input rounded-sm form-control-sm" id="map_link" value="{{ old('map_link', $serviceAddress->map_link) }}">
                            @error('map_link')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="xl:col-span-4 lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                            ...
                            <textarea name="address" class="ti-form-input rounded-sm form-control-sm" id="address" rows="3" required>{{ old('address', $serviceAddress->address) }}</textarea>
                            @error('address')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="box-footer">
                    <button type="submit" class="ti-btn ti-btn-primary-full ti-custom-validate-btn">Update</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    const existingPhone = "{{ old('contact_number', $serviceAddress->contact_number) }}";
    const existingCountryCode = "{{ old('contact_country_code', $serviceAddress->contact_country_code) }}";
    ...
    if (existingPhone && existingCountryCode) {
        itiPhone.setNumber(existingCountryCode + existingPhone);
    }
    
    // Sync country select with phone input
   
    
    // Update hidden country code on form submit
    ...
    // Rest of your country/state/city logic...
    $('#countrySelect').on('change', function() {
        const phoneCode = $(this).find('option:selected').data('phonecode');
        if (phoneCode) {
            $('#contact_country_code').val(phoneCode);
        }
